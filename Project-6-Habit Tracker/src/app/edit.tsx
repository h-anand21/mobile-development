  ...
  useEffect(() => {
    if (habit) {
      ...
      if (habit.frequency.kind === 'weekly') {
        ...
        setSelectedWeekdays([1, 2, 3, 4, 5]); // default if toggled to weekly
      }
    } else {
      Alert.alert('Error', 'Habit not found.', [{ text: 'OK', onPress: () => router.back() }]);
    }
  }, [habit]);
      ...
      <View style={[styles.root, { backgroundColor: T.bg }]}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Ionicons name="create-outline" size={18} color={T.teal} />
            <Text style={[styles.headerTitle, { color: T.textPrimary }]}>Edit Habit</Text>
          </View>
          <View style={{ width: 40 }} />
        </View>
